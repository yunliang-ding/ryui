    ...
    return children && children.map(node => {
      ...
      >
        <div className='yui-tree-icon'>
          {
            node.children ? <i className={expandedKeys.includes(node.key) ? 'iconfont-yui icon-jiantou32' : 'iconfont-yui icon-jiantou34'} /> : <i className={`iconfont-yui ${node.icon}`} style={{color: node.iconColor}} /> 
          }
        </div>
        {
